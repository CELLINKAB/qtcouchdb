...
#include <QtCouchDB/couchdbquery.h>
...
public:
    CouchDBResponse(const CouchDBQuery &query = CouchDBQuery());
    ~CouchDBResponse();
    ...
    static CouchDBResponse fromJson(const QJsonDocument &json, const CouchDBQuery &query = CouchDBQuery());
